...
public final class WaitUtils {
	...
	private static WebDriverWait wait(int sec) {
	...
	public static Boolean waitForAjax() {
		...
		return wait(ConfigReader.getInstance().getInt("timeout.seconds", 15)).until(x -> Boolean.TRUE
				.equals(((JavascriptExecutor) d).executeScript("return window.jQuery ? jQuery.active === 0 : true")));
	}

	public static void pauseForDebugOnly(long millis) {
		try {
			Thread.sleep(millis);
